...
io.on('connection', function(socket){
    ...
    socket.on('msg', function(msg){
        ...
        var X = JSON.stringify(msg.X_location); //int로 변환

        var Y = JSON.stringify(msg.Y_location); //int로 변환
        ...
        user.add_x_y_location(JSON.stringify(msg.ID) ,X, Y);//id, x좌표, y좌표
    ...
    socket.on('btn_msg', function(btn_msg){

        console.log(btn_msg);

        var msg;

        //DB에 저장한 값 불러와 클라이언트로 보내기

        user.send_x_y_location('"user1"', function(result){ //user1은 미리 DB에 저장된 유저 아이디 -> DB에 저장된 유저넘버로 바꿔서 실행해도 가능
            ...
            var msg = {
                ...
                X_location : JSON.parse(result.X_location),

                Y_location : JSON.parse(result.Y_location)
